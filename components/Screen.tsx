...
function Chip({ tone, children }: { tone: "ok" | "risk" | "warn" | "mute"; children: React.ReactNode }) {
  ...
  return (
    <span className={`num inline-flex items-center rounded px-2 py-[3px] text-[11px] font-medium ${tones[tone]}`}>
      {children}
    </span>
  );
}
  ...
  return (
    <div className="flex h-full gap-4">
      <div className="flex-1 overflow-hidden rounded-lg border border-[#e6e8ec] bg-white">
        ...
        {rows.map((r, i) => (
          <Row key={r[0]} i={i}>
            ...
            <span className="num text-[#6b7484]">{r[1]}</span>
            <span className="num text-[#6b7484]">{r[2]}</span>
            <span className="num font-semibold text-[#1a2230]">{r[3]} ₽</span>
      ...
      >
        ...
        <p className="num mt-2 text-[30px] font-semibold leading-none tracking-tight text-[#1a2230]">
          {rub(820000)} ₽
        </p>
        <div className="mt-4 border-t border-[#edeef1] pt-3">
          <span className="label text-[10px] text-[#8992a1]">Крайний срок</span>
          <p className="num mt-1 text-[15px] font-medium text-[#1a2230]">28 апреля</p>
  ...
  return (
    <div className="flex h-full gap-4">
      ...
      >
        ...
        <p className="num mt-2 text-[30px] font-semibold leading-none tracking-tight text-[#1a2230]">5 / 5</p>
        <p className="mt-1 text-[12px] text-[#0f7a55]">Всё сдано в срок</p>
        <div className="mt-4 border-t border-[#edeef1] pt-3">
          <span className="label text-[10px] text-[#8992a1]">Просрочек</span>
          <p className="num mt-1 text-[15px] font-medium text-[#1a2230]">0</p>
          <p className="mt-3 text-[12px] leading-snug text-[#6b7484]">
  ...
  return (
    <div className="flex h-full gap-4">
      <div className="flex-1 rounded-lg border border-[#e6e8ec] bg-white p-5">
        <div className="flex items-start justify-between border-b border-[#edeef1] pb-4">
          <div>
            ...
            <p className="num mt-1 text-[17px] font-semibold text-[#1a2230]">№ 4718 от 12.04</p>
        ...
        <div className="mt-4 space-y-0">
          {steps.map(([name, time, done], i) => (
            ...
            >
              ...
              <span className="num text-[12px] text-[#8992a1]">{time}</span>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      >
        ...
        <p className="num mt-2 text-[30px] font-semibold leading-none tracking-tight text-[#1a2230]">0 мин</p>
        <div className="mt-4 border-t border-[#edeef1] pt-3">
          <span className="label text-[10px] text-[#8992a1]">Срок ответа</span>
          <p className="num mt-1 text-[15px] font-medium text-[#1a2230]">5 рабочих дней</p>
          <p className="mt-3 text-[12px] leading-snug text-[#6b7484]">
      ...
      <div className="flex-1 overflow-hidden rounded-lg border border-[#e6e8ec] bg-white">
        ...
        {rows.map((r, i) => (
          <Row key={r[0]} i={i}>
            ...
            <span className="num text-[#6b7484]">{r[2]}</span>
            <span className="num text-[#6b7484]">{r[3]}</span>
            <span className="num font-semibold text-[#1a2230]">{r[4]}</span>
          </Row>
        ))}
      </div>
      <motion.aside
      ...
      <div className="flex flex-1 flex-col rounded-lg border border-[#e6e8ec] bg-white p-5">
        <div className="flex items-baseline justify-between">
          <div>
            ...
            <p className="num mt-1 text-[26px] font-semibold leading-none text-[#1a2230]">
              {revenue.toLocaleString("ru-RU")} млн ₽
            </p>
          </div>
          <div className="text-right">
            <span className="label text-[10px] text-[#8992a1]">Порог НДС</span>
            <p className="num mt-1 text-[15px] font-medium text-[#1a2230]">{limit} млн ₽</p>
          </div>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex justify-between">
            <span className="num text-[11px] text-[#8992a1]">0</span>
            <span className="num text-[11px] text-[#8a5a00]">{Math.round(pct)} % порога</span>
            <span className="num text-[11px] text-[#8992a1]">20 млн</span>
          </div>
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col p-5">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="num mt-2 text-[42px] font-semibold leading-none tracking-tight">
              {d.metric}
            </p>
            ...
            <div className="mt-5 border-t border-[#edeef1]">
              {d.rows.map(([name, value, tone], i) => (
                ...
                >
                  ...
                  <span
                    className={`num shrink-0 text-[19px] font-medium ${
                      tone === "risk"
                        ? "text-[#c02233]"
                        : tone === "warn"
                          ? "text-[#8a5a00]"
...
export default function Screen({ step, compact = false }: { step: number; compact?: boolean }) {
  ...
  return (
    <div
      aria-hidden="true"
      className="flex h-full w-full flex-col bg-[#f2f3f5] text-[#1a2230] select-none"
    >
      {/* Строка окна */}
      <div className="flex items-center gap-3 border-b border-[#e0e3e8] bg-[#e9ebef] px-4 py-[9px]">
        ...
        <span className="ml-auto flex items-center gap-2">
          ...
          <span className="num text-[11px] text-[#77808f]">синхронизировано</span>
        </span>
      </div>
            ...
            >
              {[0, 2].map((k) => (
                <span key={k} className="flex items-baseline gap-2">
                  ...
                  <span className="num text-[12px] font-medium text-[#4a5361]">
                    {statusBar[safe][k + 1]}
                  </span>
                </span>
              ))}
              <span className="num ml-auto text-[11px] text-[#9aa2b0]">
